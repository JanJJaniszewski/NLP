    ...
    def train(self, train_examples, model):
        ...
        for _ in trange(int(self.config.num_train_epochs), desc="Epoch"):
            ...
            if valid_loss == min(self.validation_losses):
                ...
                try:
                    os.remove(f'{self.config.model_dir}/temporary{str(best_model)}')
                except:
                    print('No best model found')
                torch.save({'epoch': str(i), 'state_dict': model.state_dict()},
                           f'{self.config.model_dir}/temporary{str(i)}')
                best_model = i

        # Save a trained model and the associated configuration
        checkpoint = torch.load(f'{self.config.model_dir}/temporary{str(best_model)}')
